...
export const SocietyDashboardView: React.FC<SocietyDashboardViewProps> = ({
  ...
  const completedJobsCount = serviceVisits.reduce((sum, v) => sum + (v.status === 'COMPLETED' ? v.joinedCount : 0), 0); // Approx since we only have joinedCount per visit
      ...
      <section className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-8">
        <div className="bg-white border border-slate-200 rounded-2xl p-4 shadow-xs">
          <div className="flex items-center justify-between text-neutral-500 mb-2">
            ...
            <span className="material-symbols-outlined text-lg text-neutral-700">
              calendar_month
            </span>
          </div>
          <div className="text-3xl font-black text-neutral-900">{activeVisitsCount}</div>
          <p className="text-[11px] text-emerald-700 font-medium mt-1">
            Current scheduled service blocks
          </p>
        </div>
        ...
        <div className="bg-white border border-slate-200 rounded-2xl p-4 shadow-xs">
          <div className="flex items-center justify-between text-neutral-500 mb-2">
            ...
            <span className="material-symbols-outlined text-lg text-emerald-600 fill">
              check_circle
            </span>
          </div>
      ...
      <section className="space-y-4 mb-8">
        <div className="flex items-center justify-between">
          <h2 className="font-bold text-xl text-neutral-900">
            Active &amp; Scheduled Service Visits
          </h2>
          <span className="text-xs text-neutral-500">
            {serviceVisits.length} visits active
          </span>
        </div>
        ...
        <div className="space-y-4">
          {serviceVisits.map((visit) => {
            const progress = Math.min(
              100,
              Math.round((visit.joinedCount / visit.targetCount) * 100)
            );
            return (
              <div
                key={visit.id}
                className="bg-white border border-slate-200 rounded-2xl p-5 shadow-xs flex flex-col md:flex-row justify-between gap-5"
              >
                {/* Left: Info */}
                <div className="flex gap-4 items-start">
                  <div className="w-12 h-12 rounded-xl bg-slate-100 border border-slate-200 flex items-center justify-center shrink-0">
                    <span className="material-symbols-outlined text-2xl text-neutral-800">
                      {visit.category.toLowerCase().includes('ac') ||
                      visit.category.toLowerCase().includes('appliance')
                        ? 'ac_unit'
                ...
                <div className="flex md:flex-col justify-end gap-2 shrink-0 pt-2 md:pt-0 border-t md:border-t-0 border-slate-100">
                  ...
                  <button
                    ...
                    className="flex-1 md:flex-none px-4 py-2 bg-black text-white rounded-xl text-xs font-semibold hover:bg-neutral-800 transition-colors flex items-center justify-center gap-1.5"
                  >
